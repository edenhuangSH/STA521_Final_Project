...
train = ames_train %>%  mutate(has.alley = ifelse(!is.na(Alley), 1, 0)) %>%
                        ...
                        #filter(!is.na(Bsmt.Qual)) %>% 
                        #filter(!is.na(Bsmt.Cond)) %>% 
                        #filter(!is.na(Bsmt.Exposure)) %>% 
                        #filter(!is.na(BsmtFin.Type.1)) %>% 
                        #filter(!is.na(BsmtFin.Type.2)) %>% 
                        #filter(!is.na(Garage.Yr.Blt)) %>% 
                        #filter(!is.na(Garage.Finish)) %>% 
                        #filter(!is.na(Garage.Qual)) %>%
                        #filter(!is.na(Garage.Cond)) %>%
                        select(-Alley, -Pool.QC, -Fence, -Misc.Feature, -Fireplace.Qu, -Lot.Frontage, -Garage.Type, -Bsmt.Qual, -Bsmt.Cond, -Bsmt.Exposure, -BsmtFin.Type.1, -BsmtFin.Type.2, -Garage.Yr.Blt, -Garage.Finish, -Garage.Qual, -Garage.Cond) %>% filter(!is.na(Bsmt.Full.Bath)) %>%
...
#Maximum Deviation
max(abs(Y - exp(Yhat[,1])))

#Mean Absolute Deviation
mean(abs(Y - exp(Yhat[,1])))

#RMSE
sqrt(mean((Y - exp(Yhat[,1]))^2))

#Coverage
mean(exp(predictions$lwr) < Y & exp(predictions$upr > Y))
```
