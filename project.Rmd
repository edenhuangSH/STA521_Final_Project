...
ames_train = ames_train %>% filter(!is.na(Garage.Type))%>%
  ...
  mutate(has.alley = ifelse(!is.na(Alley), 1, 0)) %>%
  ...
  select(-Alley, -Pool.QC, -Fence, -Misc.Feature, -Fireplace.Qu, -Lot.Frontage, -Sale.Condition)
```
