...
#Need to update this code... not working
ames_train[is.na("Alley"),"Alley"] <- "None"
#Need better way to recode these using indicator variables
ames_train$Exter.Qual = as.integer(as.character(ames_train$Exter.Qual))
ames_train$Exter.Cond = as.integer(as.character(ames_train$Exter.Cond))
ames_train$Bsmt.Qual = as.integer(as.character(ames_train$Bsmt.Qual)) - 1
ames_train$Bsmt.Cond = as.integer(as.character(ames_train$Bsmt.Cond)) - 1
ames_train$Bsmt.Exposure = as.integer(as.character(ames_train$Bsmt.Exposure)) - 1

#Consider putting Year.Built in age terms
train = ames_train %>% mutate(ms.subclass = factor(MS.SubClass)) %>%
                        ...
                        select(-MS.SubClass, -Pool.QC, -Fence, -Misc.Feature)
