...
model.BIC=lm(formula = log(price) ~ log(Lot.Area) + distance + Condition.1 + 
=======
model.BIC=lm(formula = log(price) ~ log(Lot.Area) + Neighborhood + Condition.1 + 
>>>>>>> e72516d6f758a1f3d1e331f8b37cae8d857af08d
...
model.interact.reduced=lm(log(price) ~ log(Lot.Area) + Condition.1 + Overall.Qual + 
    Baths + distance + Garage.Cars + log(Total.Bsmt.SF) + 
...
boost = gbm(log(price) ~ log(Lot.Area) + Condition.1 + Overall.Qual + 
            ...
            Paved.Drive + Overall.Qual:Garage.Cars + Overall.Qual:log(TotalSq) + 
            Garage.Cars:log(TotalSq) + Overall.Qual:Garage.Cars:log(TotalSq), 
            data = data_train, data = data_train, distribution="gaussian", n.trees=5000, interaction.depth = 1, shrinkage=0.01, verbose = F)
...
# neighborhoods = unique(data_train$Neighborhood)
# distance = c(1.0, 2.8, 2.8, 5.0, 2.8, 2.4, 3.0, 2.5, 4.5, 2.5, 3.0, 2.7, 1.0, 2.0, 2.7, 3.2, 2.9, 4.0, 3.2, 3.9, 1.0, 1.8, 1.3, 3.6, 2.3, 1.1, 3.4, 1.5)
# ames_train$distance = 0
# ames_train$distance[ames_train$Neighborhood=="Blmngtn"] = 3.4
# ames_train$distance[ames_train$Neighborhood=="Blueste"] = 1.1
# ames_train$distance[ames_train$Neighborhood=="BrDale"] = 2.0
# ames_train$distance[ames_train$Neighborhood=="BrkSide"] = 1.0
# ames_train$distance[ames_train$Neighborhood=="ClearCr"] = 3.6
# ames_train$distance[ames_train$Neighborhood=="CollgCr"] = 1.0
# ames_train$distance[ames_train$Neighborhood=="Crawfor"] = 2.8
# ames_train$distance[ames_train$Neighborhood=="Edwards"] = 2.8
# ames_train$distance[ames_train$Neighborhood=="Gilbert"] = 4.5
# ames_train$distance[ames_train$Neighborhood=="Greens"] = 1.3
# ames_train$distance[ames_train$Neighborhood=="GrnHill"] = 2.9
# ames_train$distance[ames_train$Neighborhood=="IDOTRR"] = 2.3
# ames_train$distance[ames_train$Neighborhood=="Landmrk"] = 1.5
# ames_train$distance[ames_train$Neighborhood=="MeadowV"] = 3.2
# ames_train$distance[ames_train$Neighborhood=="Mitchel"] = 5.0
# ames_train$distance[ames_train$Neighborhood=="NAmes"] = 3.0
# ames_train$distance[ames_train$Neighborhood=="NoRidge"] = 2.4
# ames_train$distance[ames_train$Neighborhood=="NPkVill"] = 2.5
# ames_train$distance[ames_train$Neighborhood=="NridgHt"] = 2.7
# ames_train$distance[ames_train$Neighborhood=="NWAmes"] = 2.7
# ames_train$distance[ames_train$Neighborhood=="OldTown"] = 2.5
# ames_train$distance[ames_train$Neighborhood=="Sawyer"] = 2.9
# ames_train$distance[ames_train$Neighborhood=="SawyerW"] = 2.8
# ames_train$distance[ames_train$Neighborhood=="Somerst"] = 3.2
# ames_train$distance[ames_train$Neighborhood=="StoneBr"] = 3.0
# ames_train$distance[ames_train$Neighborhood=="SWISU"] = 1.0
# ames_train$distance[ames_train$Neighborhood=="Timber"] = 4.0
# ames_train$distance[ames_train$Neighborhood=="Veenker"] = 1.8
...
plot(lm(log(price)~distance, data=ames_train))
plot(ames_train$distance, log(ames_train$price))
```
...
library(BAS)
model.bma = bas.lm(log(price) ~ log(Lot.Area) + distance + Condition.1 + 
                     Overall.Qual + Overall.Cond + HouseAge + Foundation + Bsmt.Qual + 
                     Bsmt.Exposure + log(Total.Bsmt.SF) + Heating.QC + Central.Air + 
                     Baths + Bedroom.AbvGr + Kitchen.AbvGr + Kitchen.Qual + Functional + 
                     Fireplaces + Garage.Cars + Paved.Drive + log(Pool.Area) + 
...
y_pred<-predict.bas(model.interact.reduced, newdata=data_test, estimator="BPM", prediction=TRUE, se.fit=TRUE)
...
library(ggplot2)
#Data Vizualizations
neighborhood.price.range = ggplot(data_train, aes(y = log(price), x = reorder(Neighborhood, desc(log(price)), median))) + 
                           geom_violin() + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
...
housing.ridge = lm.ridge(log(price) ~ (log(Lot.Area) + Neighborhood + Condition.1 + 
    Overall.Qual + Overall.Cond + HouseAge + Foundation + Total.Bsmt.SF + 
    Central.Air + log(X12.SF) + Baths + Kitchen.AbvGr + Kitchen.Qual + 
    Functional + Fireplaces + Garage.Cars + Paved.Drive)^2 + Bsmt.YN:Bsmt.Exposure, 
...
# lasso:
# X and Y:
X = model.matrix(log(price)~ (log(Lot.Area) + Neighborhood + Condition.1 + 
    Overall.Qual + Overall.Cond + HouseAge + Foundation + Total.Bsmt.SF + 
    Central.Air + log(X12.SF) + Baths + Kitchen.AbvGr + Kitchen.Qual + 
    Functional + Fireplaces + Garage.Cars + Paved.Drive)^2 + Bsmt.YN:Bsmt.Exposure, 
                 data = data)
...
X = model.matrix(log(price)~ log(Lot.Area) + as.numeric(Neighborhood) + as.numeric(Condition.1) + 
    Overall.Qual + Overall.Cond + HouseAge + as.numeric(Foundation) + Total.Bsmt.SF + 
    as.numeric(Central.Air) + log(X12.SF) + Baths + Kitchen.AbvGr + as.numeric(Kitchen.Qual) + 
    as.numeric(Functional) + Fireplaces + Garage.Cars + as.numeric(Paved.Drive)^2 + as.numeric(Bsmt.YN)*as.numeric(Bsmt.Exposure), 
                 data = data)

X.scaled = scale(unlist(X), center = TRUE, scale = TRUE)
Y = unlist(data[, 3])

housing.blasso.RJ = blasso(X.scaled, Y, RJ = TRUE, verb=0)
ypred.blasso.RJ = mean(housing.blasso.RJ$mu) + as.matrix(test.scaled) %*% apply(housing.blasso.RJ$beta, 2, mean)
