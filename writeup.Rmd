...
With respect to location, we note that Ames is a college town built around Iowa State. With almost 16,000 employees, the unviersity is by far the biggest employer in town. Using Google maps, we recorded the distances from the university to the different neighborhoods. Beyond a univariable analysis, we could not find an association between price and distance to university. This is illustratred in the following boxplot where we compare prices between close and distant universities with respect to the university.
...
### RMSE
Our RMSE for the simple model evaluated on the test set comes to 15,477, which corresponds to approximately 8% of the mean house price. This implies that the RMSE is fairly small.
...
The 18 most imortant predictors are summarized in the following table.
...
One of our team members wanted to include the other visualizations he created here, even though the team didn't necessarily approve.
